...
5. Compiles via `makensis.exe` (NSIS — must be installed separately, e.g. `winget install NSIS.NSIS`; the script also falls back to electron-builder's old NSIS cache if present, but don't rely on that long-term) against a generated `.nsi` script:
