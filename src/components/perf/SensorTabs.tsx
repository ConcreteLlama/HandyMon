...
import { CARD_SX, TITLE_SX, GatheringPlaceholder, chartSx, chartFillSx, AXIS_LABEL_COLOR, groupByHardware, tempColor, GroupCard, Row, GridCell } from './cards/shared';
...
const Column = ({ children, isGrid }: { children: ReactNode; isGrid: boolean }) => (
  <Box sx={isGrid
...
export function TempsView() {
  ...
  return (
    <Column isGrid={isGrid}>
      {[...groups].map(([hw, rows]) => (
        <GridCell key={hw} isGrid={isGrid}>
          <GroupCard title={hw} cardId={sensorGroupPinId('temp', rows[0].hardwareId)}>
            {rows.map((r, i) => (
...
export function FansView() {
  ...
  return (
    <Column isGrid={isGrid}>
      {[...groups].map(([hw, rows]) => (
        <GridCell key={hw} isGrid={isGrid}>
          <GroupCard title={hw} cardId={sensorGroupPinId('fan', rows[0].hardwareId)}>
            {rows.map((f, i) => {
...
export function PowerView() {
  ...
  return (
    <Column isGrid={isGrid}>
      ...
      {[...groups].map(([hw, rows]) => (
        <GridCell key={hw} isGrid={isGrid}>
          <GroupCard title={hw} cardId={sensorGroupPinId('power', rows[0].hardwareId)}>
            {rows.map((p, i) => (
